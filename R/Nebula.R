...
#' @param eta0 sparsity parameter for gamma's for type 0 variables
#' @param eta1 sparsity parameter for gamma's for type 1 variables
...
Nebula <- function(X,type,A,H,eta0,eta1,nu,alpha,lam,mu0,sig0,pr0,alpha_sigma,beta_sigma,alpha_p,beta_p,binit=NULL)
{
  n = nrow(X)
  p = ncol(X)
  idx0 = which(type==0)
  ...
  while (TRUE)
  {
    ...
    for ( h in 1:H )
    {
      ...
      cbase[idx0,h] = cbase[idx0,h] + t(lgau0) %*% EI[,h]/2 - eta0
      cbase[idx1,h] = t(Elbin[,idx1,h]) %*% EI[,h]
      cbase[idx1,h] = cbase[idx1,h] - t(lbin0) %*% EI[,h] - eta1
    }
